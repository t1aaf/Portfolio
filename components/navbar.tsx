...
import { Github, User, Briefcase, Code2, FolderKanban, Menu, TwitterIcon, X } from 'lucide-react'
...
export function Navbar() {
  ...
  const socialItems = [
    ...
    { href: "https://x.com/_t1aaf", icon: TwitterIcon },
  ]
  ...
  return (
    <motion.nav 
      ...
      className="fixed top-0 left-0 right-0 z-50 mx-auto max-w-xl px-4 mt-2"
    >
      ...
      <AnimatePresence>
        {isOpen && (
          <motion.div
            ...
            className="md:hidden mt-2 backdrop-blur-lg bg-white/30 border border-white/50 rounded-3xl shadow-lg overflow-hidden"
          >
            <div className="py-2">
              {navItems.map((item) => (
                <Link key={item.href} href={item.href}>
